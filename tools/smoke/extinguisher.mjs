...
const out = resolve(root, 'shots/extinguisher');
...
try {
  ...
  await page.evaluate(() => {
    ...
    const original = h.use.bind(h);
    window.sprayTest = { doused: 0, removed: 0 };
    h.use = () => {
      const before = h.sim.world.totalSolidVoxels();
      const result = original();
      window.sprayTest.doused += result.doused ?? 0;
      window.sprayTest.removed += before - h.sim.world.totalSolidVoxels();
      return result;
    };
    h.sim.fire.igniteArea(h.sim.world, { x: 11, y: .35, z: 5 }, 2);
    for (let i = 0; i < 240; i++) h.sim.fire.step(h.sim.world, 1 / 60);
    window.sprayTest.initial = h.sim.fire.burningCount;
    const target = [...h.sim.fire.burningPoints()].find(p => p.position.y > .3 && p.position.y < .4);
    if (!target) throw new Error('Нет горящей поверхности настила');
    t.look(target.position.x, .4, target.position.z, 0, -Math.PI / 2 + .02);
    document.getElementById('view').dispatchEvent(new window.MouseEvent('mousedown', { button: 0, bubbles: true }));
  });
  await page.waitForFunction(() => window.sprayTest.doused > 0);
  await page.waitForFunction(() => window.tvox.renderer.scene.getObjectByName('extinguisher-jet').visible);
  await page.screenshot({ path: join(out, 'jet-hit.png') });
  await page.dispatchEvent('#view', 'mouseup', { button: 0 });
  report.spray = await page.evaluate(() => window.sprayTest);
  assert.ok(report.spray.initial > 1000);
  assert.equal(report.spray.removed, 0);
  report.checks.push('Струя гасит часть разросшегося пожара, видна и не удаляет дерево');
  await page.evaluate(() => {
    window.tvox.look(11, .4, 5, 0, 1.4);
    document.getElementById('view').dispatchEvent(new window.MouseEvent('mousedown', { button: 0, bubbles: true }));
  });
  await page.waitForFunction(() => document.getElementById('hint').textContent.includes('не достаёт'));
  await page.dispatchEvent('#view', 'mouseup', { button: 0 });
  await page.keyboard.press('Escape');
  await page.waitForSelector('#menu:not([hidden])');
  assert.equal(await page.evaluate(() => window.tvox.renderer.scene.getObjectByName('extinguisher-jet').visible), false);
  report.checks.push('При недолёте показана дальность; в меню струя скрыта');
